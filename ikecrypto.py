...
class ikeCrypto(object):
    ...
    def calcSKEYID(self, psk, rawNonce_i, rawNonce_r, hashType):
        ...
        if hashType == "md5" or hashType == "01":
            skeyid = hmac.new(psk, rawNonce_i + rawNonce_r, hashlib.md5).hexdigest()
            if debug > 0:
                print("SKEYID: %s" % skeyid)
            SKEYID = bytes.fromhex(skeyid)
    ...
    def calcHASH(self, SKEYID, rawDHPub_r, rawDHPub_i, rawCookie_r, rawCookie_i, rawSA_i, rawID_r, rawID_i, riHash,
                 hashType):
        ...
        if hashType == "md5" or hashType == "01":
            if riHash == "r":
                HASH_R = hmac.new(SKEYID, rawDHPub_r + rawDHPub_i + rawCookie_r + rawCookie_i + rawSA_i + rawID_r,
                ...
                HASH_I = hmac.new(SKEYID, rawDHPub_i + rawDHPub_r + rawCookie_i + rawCookie_r + rawSA_i + rawID_i,
                                  hashlib.md5).hexdigest()
                if debug > 0:
    ...
    def calcHASHmcfg(self, SKEYID_a, msgID, mcfgAttr, hashType):
        ...
        if hashType == "md5" or hashType == "01":
            mcfgHASH = hmac.new(SKEYID_a, msgID + mcfgAttr, hashlib.md5).hexdigest()
            if debug > 0:
    ...
    def calcHASHQM(self, SKEYID_a, msgID, data, hashType, hashNum):
        ...
        if hashType == "md5" or int(hashType) == 1:
            if hashNum == 1:
                qmHASH = hmac.new(SKEYID_a, msgID + data, hashlib.md5).hexdigest()
                if debug > 0:
        ...
        if hashType == "md5" or int(hashType) == 1:
            if hashNum == 3:
                qmHASH = hmac.new(SKEYID_a, b"\x00" + msgID + data, hashlib.md5).hexdigest()  # where data = Ni_b | Nr_b
                if debug > 0:
                    print('Quick Mode HASH3: %s' % qmHASH)
                return qmHASH
        elif hashType == "sha" or int(hashType) == 2:
            if hashNum == 3:
                qmHASH = hmac.new(SKEYID_a, b"\x00" + msgID + data,
                                  hashlib.sha1).hexdigest()  # where data = Ni_b | Nr_b
                if debug > 0:
                    print('Quick Mode HASH3: %s' % qmHASH)
                return qmHASH
    ...
    def calcHASHgen(self, SKEYID, rawData, hashType):
        ...
        if hashType == "md5" or hashType == "01":
            HASH_I = hmac.new(SKEYID, rawData, hashlib.md5).hexdigest()
            if debug > 0:
                print('HASH: %s' % HASH_I)
            return HASH
    ...
    def calcSKEYID_d(self, SKEYID, rawDHShared, rawCookie_i, rawCookie_r, hashType):
        ...
        if hashType == "md5" or hashType == "01":
            skeyid_d = hmac.new(SKEYID, rawDHShared + rawCookie_i + rawCookie_r + b"\x00", hashlib.md5).hexdigest()
            SKEYID_d = bytes.fromhex(skeyid_d)
            if debug > 0:
                print('SKEYID_d %s' % skeyid_d)
            return SKEYID_d
        elif hashType == "sha" or hashType == "02":
            skeyid_d = hmac.new(SKEYID, rawDHShared + rawCookie_i + rawCookie_r + b"\x00", hashlib.sha1).hexdigest()
            SKEYID_d = bytes.fromhex(skeyid_d)
            if debug > 0:
                print('SKEYID_d %s' % skeyid_d)
            return SKEYID_d
    ...
    def calcSKEYID_a(self, SKEYID, SKEYID_d, rawDHShared, rawCookie_i, rawCookie_r, hashType):
        # Provide values as raw decoded hex strings
        if hashType == "md5" or hashType == "01":
            skeyid_a = hmac.new(SKEYID, SKEYID_d + rawDHShared + rawCookie_i + rawCookie_r + b"\x01",
                                hashlib.md5).hexdigest()
            SKEYID_a = bytes.fromhex(skeyid_a)
            if debug > 0:
    ...
    def calcSKEYID_e(self, SKEYID, SKEYID_a, rawDHShared, rawCookie_i, rawCookie_r, hashType):
        # Provide values as raw decoded hex strings
        if hashType == "md5" or hashType == "01":
            skeyid_e = hmac.new(SKEYID, SKEYID_a + rawDHShared + rawCookie_i + rawCookie_r + b"\x02",
                                hashlib.md5).hexdigest()
            SKEYID_e = bytes.fromhex(skeyid_e)
            if debug > 0:
    ...
    def calcKa(self, SKEYID_e, keyLen, hashType):
        ...
        keyLen = keyLen * 2
        if hashType == "md5" or hashType == "01":
            k1 = hmac.new(SKEYID_e, b"\x00", hashlib.md5).hexdigest()
            K1 = bytes.fromhex(k1)
            if debug > 0:
                print("K1: %s" % k1)
            if len(k1) < keyLen:
                k2 = hmac.new(SKEYID_e, K1, hashlib.md5).hexdigest()
                k2List = (k1, k2)
                ka = ''.join(k2List)
                K2 = bytes.fromhex(k2)
                if debug > 0:
                    print("K2: %s" % k2)
                if len(ka) < keyLen:
                    k3 = hmac.new(SKEYID_e, K2, hashlib.md5).hexdigest()
                    if debug > 0:
                        print("K3: %s" % k3)
                    k3List = (k1, k2, k3)
                    ka = ''.join(k3List)
            ...
            k1 = hmac.new(SKEYID_e, b"\x00", hashlib.sha1).hexdigest()
            K1 = bytes.fromhex(k1)
            if debug > 0:
                print("K1: %s" % k1)
            if len(k1) < keyLen:
    ...
    def calcKEYMAT(self, hashType, keyLen, SKEYID_d, prot, SPI, nonce_i, nonce_r, *arg):
        ...
        try:
            ...
            if hashType == "md5" or int(hashType) == 1:
                keymat = hmac.new(SKEYID_d, arg + prot + SPI + nonce_i + nonce_r, hashlib.md5).hexdigest()
                if debug > 0:
            ...
            if hashType == "md5" or int(hashType) == 1:
                keymat = hmac.new(SKEYID_d, prot + SPI + nonce_i + nonce_r, hashlib.md5).hexdigest()
            elif hashType == "sha" or int(hashType) == 2:
            ...
            ###***make this a method
            if len(bytes.fromhex(keymat)) < keyLen:
                k1 = keymat
                k2 = hmac.new(SKEYID_d, bytes.fromhex(k1, hashlib.md5)).hexdigest()
                ka = k1 + k2
                if debug > 0:
                    print("K2: %s" % k2)
                if len(bytes.fromhex(ka)) < keyLen:
                    k3 = hmac.new(SKEYID_d, bytes.fromhex(k2, hashlib.md5)).hexdigest()
                    if debug > 0:
                        print("K3: %s" % k3)
                    ka = k1 + k2 + k3
                    ka = ka[:keyLen]
...
def main():
    ...
    rawDHPub_r = bytes.fromhex(DHPub_r)
    ...
    rawDHPub_i = bytes.fromhex(DHPub_i)
    ...
    rawSA_i = bytes.fromhex(SA_i)
    cookie_r = "25c250397161ddf3"
    rawCookie_r = bytes.fromhex(cookie_r)
    cookie_i = "1dc509539bb7fcbc"
    rawCookie_i = bytes.fromhex(cookie_i)
    ID_r = "01110000c0a83965"
    rawID_r = bytes.fromhex(ID_r)
    ID_i = "0b1101f476706e"
    rawID_i = bytes.fromhex(ID_i)
    nonce_r = "2b9d2a46590f8764a09c5f09b08f10d2cfb3f918"
    rawNonce_r = bytes.fromhex(nonce_r)
    nonce_i = "1b8f8ccb5f4ea7a42e8736bf07bfc3b95952f0ce"
    rawNonce_i = bytes.fromhex(nonce_i)
    protocol = bytes.fromhex("01")  # (ISAKMP)
    ...
    rawDHShared = bytes.fromhex(DHShared)
    ...
    HASH_R = testCrypt.calcHASH(SKEYID, rawDHPub_r, rawDHPub_i, rawCookie_r, rawCookie_i, rawSA_i, rawID_r, rawID_i,
                                "r", hashType)
    HASH_I = testCrypt.calcHASH(SKEYID, rawDHPub_r, rawDHPub_i, rawCookie_r, rawCookie_i, rawSA_i, rawID_r, rawID_i,
                                "i", hashType)
    ...
    curIV = testCrypt.calcIV(bytes.fromhex(lastBlock), bytes.fromhex(msgID), IVLen, hashType)
    ...
    HASHmcfg = testCrypt.calcHASHmcfg(SKEYID_a, bytes.fromhex(msgID), bytes.fromhex(mcfgAttr), hashType)
